...
static int read_problem_data(const std::string& filename,
		std::vector<std::vector<PREC> >& data) {
	data.clear();
...
int main(int argc, char* argv[]) {
	...
	input_options.add_options()("data",
			po::value<std::string>(&data_filename)->default_value("data.txt"),
			"Data file, one datum per line")(
			"cluster",
			po::value<std::string>(&cluster_filename)->default_value(
					"clustercenter.txt"),
			"Output file, one cluster center per line")(
			"assignment",
			po::value<std::string>(&assignment_filename)->default_value(
					"assignment.txt"),
			"Output file, one cluster center per line");
	...
	if (data_X[0].size() != data_CX[0].size()) {
		std::cerr << "Dimension mismatch between points and clusters"
				<< std::endl;
		exit(EXIT_FAILURE);
	}

	// convert clusters to PREC*
	PREC *CX = (PREC *) malloc(nof_clusters * dims * sizeof(PREC));
	unsigned int cntr = 0;
	...
	PREC *x = (PREC *) malloc(dims * sizeof(PREC));
	for (unsigned int i = 0; i < nof_points; i++) {
		for (unsigned int n = 0; n < data_X[i].size(); ++n)
			x[n] = data_X[i][n];
		labels.push_back(
				1 + assign_point_to_cluster_ordinary(x, CX, dims, nof_clusters));
	}
