...
def flattened_images_subdir(idir, odir, usergb):
    ...
    if len(sdirs) != 0 and len(limages) == 0:
        odir = os.path.join(odir, 'lfwa-flattened') + os.path.sep;
        print odir
            ...
            for i in limages:
                if os.path.splitext(i)[1] == '.jpg' or os.path.splitext(i)[1] == '.png':
                    ...
                    if im.mode=="RGB" and not usergb:
                        ...
                        im.save(dimg);
        return odir  # return the newly created flattened directory as input directory
    return idir
...
def get_cropped_images_dir_name(options):
    ...
    dirname = "cropped_%s_images_%dX%d_xoffset=%d_yoffset=%d" % (options.dataset, options.width, options.height, options.xoffset, options.yoffset)
