...
class ArgumentParser:
    ...
    def __init__(self, v=1.0):
        ...
        pgroup.add_argument("-c", "--configfile", dest="configfile",
                                 help="Please specify a config file (e.g. config_test.py or config.py),"
        ...
        pgroup.add_argument("-m", "--module", dest="module",
                                 ...
                                 + " for each module help use : python run.py -m features -h")
                   
        if len(sys.argv) == 1 or self.args[0].lstrip('-').lower() == 'h' or self.args[0].lstrip('-').lower() == 'help':
...
#            sys.exit(1)
    ...
    def execute(self):
        ...
        if not os.path.exists(cidir) or len(os.listdir(cidir)) < len(os.listdir(options.idir)):
            ...
            mimname = os.path.join(os.path.normpath(os.path.join(cidir, os.path.pardir)), 'meanim' + options.rgbsuffix + '.png')
            imsave(mimname, mim);
...
#             print options
            ...
            options.featdir = os.path.join(self.options.odir, "features" + self.options.rgbsuffix);
            return options
        except ValueError:
